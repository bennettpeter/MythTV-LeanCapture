...
# Mark recording as damaged and allow re-record
...
if [[ "$videoprop" == *DAMAGED* ]] ; then
    echo "Recording was already marked damaged"
else
    sql="UPDATE recordedprogram
    SET videoprop = '$videoprop,DAMAGED'
...
echo "Program $title on channel $channum at $localdate is now marked as DAMAGED"
